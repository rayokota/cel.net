...
//	import static org.projectnessie.cel.common.Source.newTextSource;
    ...
    public class CheckerTest
    {
        public class TestCase
        {
            ...
            /// <summary>
            /// Type is the expected type of the expression </summary>
            internal Type type;

            /// <summary>
            /// Container is the container name to use for test. </summary>
            internal string container = "";

            /// <summary>
            /// Env is the environment to use for testing. </summary>
            internal Env env;

            /// <summary>
            /// Error is the expected error for negative test cases. </summary>
            internal string error;

            /// <summary>
            /// DisableStdEnv indicates whether the standard functions should be disabled. </summary>
        ...
        internal static readonly IDictionary<string, Env> testEnvs = new Dictionary<string, Env>{{"default",
            new Env()
                .Functions(
...
//ORIGINAL LINE: @SuppressWarnings("unused") static TestCase[] checkTestCases()
    ...
    [TestCaseSource(nameof(CheckTestCases))]
        public virtual void Check(TestCase tc)
        {
            ...
            Source src = Source.NewTextSource(tc.i);
            ...
            TypeRegistry reg = ProtoTypeRegistry.NewRegistry(
                new Google.Api.Expr.Test.V1.Proto2.TestAllTypes(),
            ...
            if (tc.disableStdEnv)
            {
                env = CheckerEnv.NewCheckerEnv(cont, reg);
            }
            ...
            if (tc.env != null)
            {
                ...
                {
                    foreach (Decl ident in tc.env.idents)
                    {
                        env.Add(ident);
                    }
                }
                ...
                {
                    foreach (Decl fn in tc.env.functions)
                    {
                        env.Add(fn);
                    }
                }
            ...
            Checker.CheckResult checkResult = Checker.Check(parsed, src, env);
            if (checkResult.HasErrors())
            {
                string errorString = checkResult.Errors.ToDisplayString();
                if (!string.ReferenceEquals(tc.error, null))
                {
                    Assert.That(errorString, Is.EqualTo(tc.error));
                }
                else
                {
                    Assert.Fail(string.Format("Unexpected type-check errors: {0}", errorString));
                }
            }
            else if (!string.ReferenceEquals(tc.error, null))
            {
                Assert.That(tc.error, Is.Null);
            }

            Type actual = checkResult.CheckedExpr.TypeMap[parsed.Expr.Id];
            if (string.ReferenceEquals(tc.error, null))
            {
                if (actual == null || !actual.Equals(tc.type))
                {
                    Assert.Fail(string.Format("Type Error: '{0}' vs expected '{1}'", actual, tc.type));
                }
            }

            if (!string.ReferenceEquals(tc.r, null))
            {
                string actualStr = Printer.Print(checkResult.CheckedExpr.Expr, checkResult.CheckedExpr);
                string actualCmp = Regex.Replace(actualStr,"[ \n\t]", "");
                string rCmp = Regex.Replace(tc.r, "[ \n\t]", "");
