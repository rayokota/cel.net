...
//ORIGINAL LINE: @SuppressWarnings("unchecked") @Override public <T> T convertToNative(Class<T> typeDesc)
	  ...
	  {
		if (typeDesc.IsAssignableFrom(typeof(long)) || typeDesc.IsAssignableFrom(typeof(long)) || typeDesc == typeof(object))
		{
			return value;
		}
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		throw new Exception(String.Format("native type conversion error from '{0}' to '{1}'", UnknownType, typeDesc.FullName));
	  }
