	...
	public sealed class CheckerEnv
	{
	  ...
	  internal Decl AddOverload(Decl f, Decl.Types.FunctionDecl.Types.Overload overload, IList<string> errMsgs)
	  {
		...
		foreach (Decl.Types.FunctionDecl.Types.Overload existing in function.Overloads)
		{
		  ...
		  {
			errMsgs.Add(OverlappingOverloadError(f.Name, overload.OverloadId, overloadFunction, existing.OverloadId, existingFunction));
			hasErr = true;
		  }
		}
		...
		{
		  if (macro.Function().Equals(f.Name) && macro.ReceiverStyle == overload.IsInstanceFunction && macro.ArgCount() == overload.Params.Count)
		  {
	  ...
	  internal Decl SanitizeFunction(Decl decl)
	  {
		...
		IList<Decl.Types.FunctionDecl.Types.Overload> overloads = new List<Decl.Types.FunctionDecl.Types.Overload>(fn.Overloads);
		foreach (Decl.Types.FunctionDecl.Types.Overload o in fn.Overloads)
		{
